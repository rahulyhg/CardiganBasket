...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Page Builder', AQPB_TEXT_DOMAIN_SLUG);
			$defaults['page_title'] = __('Page Builder', AQPB_TEXT_DOMAIN_SLUG);
			$defaults['page_slug'] = __('aq-page-builder', AQPB_TEXT_DOMAIN_SLUG);
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', AQPB_TEXT_DOMAIN_SLUG) .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', AQPB_TEXT_DOMAIN_SLUG);
				echo '</p>';
				return;
...
------------------------------------------------------------------------------------------------------------------- */
				$hook_args = array(
					...
					'selected_template' => ( isset( $_GET['preset_template']) ? esc_attr($_GET['preset_template']) : ( isset( $_POST['preset_template']) ? esc_attr($_POST['preset_template']) : NULL ) ),
				);
				do_action( 'aqpb_create_template_hook' , $hook_args );
/* END Custom Stuff
------------------------------------------------------------------------------------------------------------------- */
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo __('This template is empty', AQPB_TEXT_DOMAIN_SLUG);
				echo '</p>';
				
			} else {

//global $aqpb_customclass;
//echo $aqpb_customclass->retrieve_meta_info($template_id , true) . '<br /><br />' . $aqpb_customclass->retrieve_meta_info($template_id , false);
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Aqua Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', AQPB_TEXT_DOMAIN_SLUG) .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . AQPB_DIR . '/assets/images/aqua-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', AQPB_TEXT_DOMAIN_SLUG )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', AQPB_TEXT_DOMAIN_SLUG )  . '">' . $img . '</a>';
				} else {
					...
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', AQPB_TEXT_DOMAIN_SLUG ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', AQPB_TEXT_DOMAIN_SLUG ) . '</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						...
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', AQPB_TEXT_DOMAIN_SLUG ) ); ?>");
							return;
						}
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', AQPB_TEXT_DOMAIN_SLUG ), '<a href="' .admin_url().'themes.php?page=aq-page-builder">', '</a>' );
							return;
						}
						?>						

						<h3><?php _e( 'Choose Your Page Template', AQPB_TEXT_DOMAIN_SLUG ); ?></h3><br />
						...
						<input type="button" id="aqpb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', AQPB_TEXT_DOMAIN_SLUG ); ?>" onclick="insertTemplate();" />
						<a id="aqpb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', AQPB_TEXT_DOMAIN_SLUG ); ?>"><?php echo esc_attr__( 'Cancel', AQPB_TEXT_DOMAIN_SLUG ); ?></a>
