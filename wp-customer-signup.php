...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	...
	<h2><?php printf( __( 'Create your customer account' ), get_current_site()->site_name ) ?></h2>
	<form id="setupform" method="post" action="wp-signup.php">
...
<!-- MAKE INPUT INVISIBLE BUT SELECTED -->
			<input id="signupuser" type="hidden" name="signup_for" value="user" <?php checked( $signup_for, 'user' ); ?> />
			<!--<label class="checkbox" for="signupuser"><?php _e('Just a username, please.') ?></label> -->
		<?php } ?>
		</p>

		<p class="submit"><input type="submit" name="submit" class="submit" value="<?php esc_attr_e('Submit') ?>" /></p>
	</form>
	<?php
}
...
function confirm_user_signup($user_name, $user_email) {
	?>
	<h2><?php printf( __( '%s is your customer username' ), $user_name) ?></h2>
	<p><?php _e( 'But, before you can start using your new account, <strong>you must activate it</strong>.' ) ?></p>
...
function confirm_blog_signup( $domain, $path, $blog_title, $user_name = '', $user_email = '', $meta = array() ) {
	?>
	<h2><?php printf( __( 'Thank you for joining as a customer! Your new customer account is almost ready.' ), "<a href='http://{$domain}{$path}'>{$blog_title}</a>" ) ?></h2>

	<p><?php _e( 'But, before you can start using your customer account, <strong>you must activate it</strong>.' ) ?></p>
	<p><?php printf( __( 'Check your inbox at <strong>%s</strong> and click the link given.' ),  $user_email) ?></p>
