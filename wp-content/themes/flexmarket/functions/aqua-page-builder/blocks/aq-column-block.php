...
class AQ_Column_Block extends AQ_Block {
				...
				'<dl class="block-bar">',
					'<dt class="block-handle">',
						...
						'<span class="block-controls">',
							'<a class="block-edit" id="edit-'.$number.'" title="Edit Block" href="#block-settings-'.$number.'">' . __('Edit Block', AQPB_TEXT_DOMAIN_SLUG ) . '</a>',
						'</span>',
					'</dt>',
				'</dl>',
	...
	function form($instance) {
		...
		__('Drag block items into this column box', AQPB_TEXT_DOMAIN_SLUG ),
		'</p>';
	...
	function after_form($instance) {
		...
		$block_saving_id = 'aq_blocks[aq_block_'.$number.']';
			...
			echo __( 'Class (optional)' , AQPB_TEXT_DOMAIN_SLUG  ) . '<br/><input type="text" class="widefat" name="'.$this->get_field_name('userclass').'" value="'.$userclass.'" />';
			echo '<div class="cf" style="height: 10px"></div>';
			echo __( 'Additional inline css styling (optional)' , AQPB_TEXT_DOMAIN_SLUG  ) . '<br/><input type="text" class="widefat" name="'.$this->get_field_name('style').'" value="'.$style.'" />';
			echo '<div class="cf" style="height: 10px"></div>';
	...
	function block_callback($instance) {
		...
		if(isset($template_id)) {
			...
			// additional options
			$userclass = (!empty($userclass) ? ' class="column-block-well ' . esc_attr($userclass).'"' : '' );
	 		$style = (!empty($style) ? ' style="' . esc_attr($style).'"' : '');

	 		echo '<div'.$userclass.$style.'>';
